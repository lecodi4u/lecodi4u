...
.added {
	...
	<jsp:include page="../header.jsp"></jsp:include>
		<h3>장바구니</h3>
		<c:if test="${sessionScope.cartList.size() == 0}"><!-- 장바구니에 담긴 물건이 없는 경우 -->
			장바구니에 담긴 물건이 없습니다.
		</c:if>
		<c:if test="${sessionScope.cartList.size() != 0}"><!-- 장바구니에 담긴 물건이 있는 경우 -->
			<table style="width: 500px">
				...
				<c:forEach items="${sessionScope.cartList}" var="cart">
					<tr>
						...
						<td>${cart.color}</td>
						<td>${cart.c_size}</td>
						<td>${cart.cartmount}</td>
						<td><input type="checkbox" selectnum="${cart.productcode}"></td>
					</tr>
				</c:forEach>
			</table>
